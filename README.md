...
(𝑼𝐨𝐮𝐭) under varying load and reference conditions.

A two-loop cascaded control structure is used:
• Inner current control loop (𝑮𝐜𝐜(𝒔)): regulates the inductor current 𝑖𝐿 to follow the reference 𝑖Lref.
• Outer voltage control loop (𝑮𝐜𝐯(𝒔)): generates the reference 𝑖Lref to regulate the output voltage Uout.
...
• Reference change (step in 𝑈outref)
